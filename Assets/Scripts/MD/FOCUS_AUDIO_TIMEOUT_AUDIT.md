...
|---|---|---|---|---|
| 1 | Visibility listener registration (.jslib + wrapper + Awake) | FIXED (was MISSING) | `Assets/Plugins/WebGL/CustomJsLib.jslib`, `Assets/Scripts/JSFunctCalls.cs`, `Assets/Scripts/UI/UIManager.cs` (Awake) | Added `RegisterVisibilityChangeListener` to jslib, `RegisterVisibilityListener` wrapper, `jsFunctCalls` field + `Awake()` call on `UIManager` (GameObject `UIMananger`). Field left unassigned in Inspector — must be wired manually (see below) |
| 2 | `OnFocusChanged` public callback | FIXED (was MISSING) | `Assets/Scripts/UI/UIManager.cs` (`OnFocusChanged`) | Added `public void OnFocusChanged(string value)`, routes to `audioController.SetMuteAll` + `socketManager.HandleFocusChange` |
| 3 | Audio mute/unmute honors user sound setting (both focus paths call one shared mute-flag method, reentrancy-guarded; invariant + reverse invariant; no Pause/UnPause) | FIXED (was FAIL — used `AudioSource.Pause()`/`UnPause()`) | `Assets/Scripts/Functionality/AudioController.cs` (`SetMuteAll`), `Assets/Scripts/Functionality/SlotBehaviour.cs` (`OnApplicationFocus`) | Removed `CheckFocusFunction`/`RecieveReactNativeAudioChanges` (Pause/UnPause pattern); added reentrancy-guarded, per-`AudioSource` `SetMuteAll(bool)`; both `UIManager.OnFocusChanged` and `SlotBehaviour.OnApplicationFocus` now call it |
| 4 | 60s background timeout (`WaitForSecondsRealtime`) | FIXED (was MISSING) | `Assets/Scripts/APIs/SocketIOManager.cs` (`HandleFocusChange`, `FocusTimeoutCheck`) | Added fields, `HandleFocusChange`, `FocusTimeoutCheck` coroutine (`WaitForSecondsRealtime`), `OnDestroy` guard; reuses existing `ResetPingRoutine()` and `uiManager.DisconnectionPopup()` |
| 5 | `OnError` (session-expired vs generic) | PASS | `Assets/Scripts/APIs/SocketIOManager.cs` (`OnError`) | No action — already matches reference |
| 6 | Instant JS-side mute (WebAudio suspend in `.jslib`) | FIXED (was MISSING) | `Assets/Plugins/WebGL/CustomJsLib.jslib` | Folded into Check 1's `RegisterVisibilityChangeListener` addition (`setUnityAudioSuspended` inside `sendFocusToUnity`) |
| 7 | Orientation change / responsive scaling (`SwitchDisplay` + continuous match) | PASS | `Assets/Scripts/OrientationChange.cs` (GameObject `OC`) | No action — matches Age of Gods reference verbatim |
| 8 | WebGL template canvas fit (`resizeCanvas` + listeners, macros intact) | FIXED (was FAIL) | `Assets/WebGLTemplates/custom/index.html` | Replaced hardcoded 16:9 letterbox `resizeCanvas` with reference `visualViewport`-based fit; fixed `addEventListener('load', resizeCanvas())` → `resizeCanvas`; added `orientationchange`/`visualViewport`/scroll-lock listeners. `#if`/`#endif` macros and template tokens were already intact — no change needed there |
| 9 | No blanket console suppression in WebGL template | FIXED (was FAIL) | `Assets/WebGLTemplates/custom/index.html` | Removed the `console.log`/`warn`/`error` no-op override block |
| 10 | Balance sync socket event (`balance:sync`) | FIXED (was MISSING) | `Assets/Scripts/APIs/SocketIOManager.cs` (`BalanceSyncPayload`, `OnBalanceSync`), `Assets/Scripts/Functionality/SlotBehaviour.cs` (`UpdateBalanceDisplay`) | Added DTO + `gameSocket.On<string>("balance:sync", OnBalanceSync)` registration + handler; writes into `PlayerData.balance` and calls new `SlotBehaviour.UpdateBalanceDisplay`, which updates `Balance_text` and re-runs `CompareBalance()` |
| 11 | No high-frequency/heartbeat debug logging | PASS | `Assets/Scripts/APIs/SocketIOManager.cs` (`PingCheck`, `OnPongReceived`) | No action — success-path logs already commented out |

**Manual step still required:** in the Unity Editor, select the `UIMananger` GameObject and drag
`CustomJSFuncManager` onto the new `Js Funct Calls` field on its `UIManager` component. Checks 1/2/6
are wired in code but inert until that Inspector reference is assigned.
